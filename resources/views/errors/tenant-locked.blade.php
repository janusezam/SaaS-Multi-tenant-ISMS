    ...
    <body class="isms-theme antialiased">
        @php
            ...
            $statusClasses = match ($status) {
                ...
                'expired' => 'bg-rose-500/20 text-rose-100 border-rose-300/30',
                'suspended' => 'bg-amber-500/20 text-amber-100 border-amber-300/30',
        ...
        <div class="min-h-screen px-4 py-10 sm:px-6 lg:px-8">
            <div class="mx-auto max-w-3xl">
                <div class="overflow-hidden rounded-2xl border border-amber-300/30 bg-slate-950/70 shadow-2xl shadow-slate-950/40 backdrop-blur">
                    <div class="border-b border-white/10 px-6 py-5 sm:px-8">
                        <p class="text-xs font-semibold uppercase tracking-[0.2em] text-amber-200">Subscription Locked</p>
                        <h1 class="mt-2 text-2xl font-bold text-slate-100 sm:text-3xl">Your school account is temporarily locked</h1>
                    </div>

                    <div class="space-y-4 px-6 py-6 sm:px-8 sm:py-8">
                        <div class="grid gap-3 rounded-xl border border-white/10 bg-slate-950/60 p-4 sm:grid-cols-2">
                            <div>
                                <p class="text-xs uppercase tracking-wider text-slate-400">School</p>
                                <p class="mt-1 text-sm font-semibold text-slate-100">{{ $tenantName ?? 'Your school' }}</p>
                            </div>
                            <div>
                            ...
                            @if (! empty($tenantDueDate))
                                <div class="sm:col-span-2">
                                    ...
                                    <p class="mt-1 text-sm font-semibold text-slate-100">{{ $tenantDueDate->format('F j, Y') }}</p>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="flex flex-wrap gap-3 pt-2">
                            <a href="{{ route('tenant.login') }}" class="inline-flex items-center rounded-lg border border-white/20 px-4 py-2 text-sm font-semibold text-slate-100 transition hover:bg-white/10">
                                Back to Login
                            </a>
