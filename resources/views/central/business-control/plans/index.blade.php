    ...
    <div class="business-control-page mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8" x-data="{ createOpen: @js($openCreate), editOpen: @js($openEditId) }">
        ...
        <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
            @forelse ($plans as $plan)
                ...
                <article class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-6 text-slate-900 shadow-sm dark:border-cyan-300/20 dark:bg-[linear-gradient(165deg,rgba(2,6,23,0.97)_0%,rgba(10,18,36,0.98)_52%,rgba(8,15,30,0.99)_100%)] dark:text-slate-100 dark:shadow-[0_0_0_1px_rgba(34,211,238,0.05),0_20px_36px_rgba(2,6,23,0.72)]">
                    <div class="pointer-events-none absolute -right-12 -top-14 h-40 w-40 rounded-full bg-cyan-200/35 blur-3xl dark:bg-cyan-400/5 dark:blur-3xl"></div>
                    <div class="relative">
                        ...
                        <div class="mt-6 flex items-center justify-between gap-2 text-xs">
                            ...
                            <span class="rounded-full border px-2.5 py-1 {{ $plan->hasFeature('exports') ? 'border-cyan-200 bg-cyan-50 text-cyan-800 dark:border-cyan-300/40 dark:bg-cyan-400/15 dark:text-cyan-100' : 'border-slate-300 bg-slate-100 text-slate-700 dark:border-slate-300/30 dark:bg-slate-500/20 dark:text-slate-200' }}">Exports {{ $plan->hasFeature('exports') ? 'On' : 'Off' }}</span>
                        </div>
                        ...
                        <div class="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-cyan-200/15 dark:bg-slate-950/70">
                ...
                <form method="POST" action="{{ route('central.business-control.plans.store') }}" class="grid gap-3 md:grid-cols-3">
                    ...
                    <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 md:col-span-3 dark:border-white/10 dark:bg-slate-950/40">
                        ...
                        <div class="mt-2 grid gap-2 sm:grid-cols-2">
                            ...
                            <label class="inline-flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 sm:col-span-2">
                                <input type="hidden" name="feature_flags[exports]" value="0">
                                <input type="checkbox" name="feature_flags[exports]" value="1" @checked((bool) old('feature_flags.exports', false)) class="rounded border-slate-300 bg-white text-cyan-600 dark:border-white/20 dark:bg-slate-900 dark:text-cyan-500">
                                Export CSV / PDF
                            </label>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('central.business-control.plans.update', $plan) }}" class="grid gap-3 md:grid-cols-3">
                        ...
                        <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 md:col-span-3 dark:border-white/10 dark:bg-slate-950/40">
                            ...
                            <div class="mt-2 grid gap-2 sm:grid-cols-2">
                                ...
                                <label class="inline-flex items-center gap-2 text-sm text-slate-700 dark:text-slate-300 sm:col-span-2">
                                    <input type="hidden" name="feature_flags[exports]" value="0">
                                    <input type="checkbox" name="feature_flags[exports]" value="1" @checked((bool) ($isEditingThisPlan ? old('feature_flags.exports', $plan->hasFeature('exports')) : $plan->hasFeature('exports'))) class="rounded border-slate-300 bg-white text-cyan-600 dark:border-white/20 dark:bg-slate-900 dark:text-cyan-500">
                                    Export CSV / PDF
                                </label>
                            </div>
                        </div>
