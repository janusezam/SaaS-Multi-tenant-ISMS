<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold text-slate-900 dark:text-slate-100">Edit School</h2>
    </x-slot>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 dark:border-white/10 dark:bg-slate-900/85">
            <form method="POST" action="{{ route('central.universities.update', $university) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input id="name" name="name" value="{{ old('name', $university->name) }}" class="w-full rounded-xl border border-slate-300 bg-white text-slate-900 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100" required />
                ...
                <div>
                    ...
                    <input id="school_address" name="school_address" value="{{ old('school_address', $university->school_address) }}" class="w-full rounded-xl border border-slate-300 bg-white text-slate-900 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100" required />
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input id="tenant_admin_name" name="tenant_admin_name" value="{{ old('tenant_admin_name', $university->tenant_admin_name) }}" class="w-full rounded-xl border border-slate-300 bg-white text-slate-900 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100" required />
                    ...
                    <div>
                        ...
                        <input id="tenant_admin_email" type="email" name="tenant_admin_email" value="{{ old('tenant_admin_email', $university->tenant_admin_email) }}" class="w-full rounded-xl border border-slate-300 bg-white text-slate-900 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100" required />
                ...
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                    <div>
                        ...
                        <select id="plan" name="plan" class="w-full rounded-xl border border-slate-300 bg-white text-slate-900 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100" required>
                            @foreach ($plans as $plan)
                                <option value="{{ $plan->code }}" @selected(old('plan', $university->plan) === $plan->code)>{{ $plan->name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        <label class="mb-2 block text-sm text-slate-700 dark:text-slate-300" for="status">Status</label>
                        <select id="status" name="status" class="w-full rounded-xl border border-slate-300 bg-white text-slate-900 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100" required>
                    ...
                    <div>
                        ...
                        <input id="subscription_starts_at" type="date" name="subscription_starts_at" value="{{ old('subscription_starts_at', $university->subscription_starts_at?->format('Y-m-d')) }}" class="w-full rounded-xl border border-slate-300 bg-white text-slate-900 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100" />
                        @error('subscription_starts_at')<p class="mt-1 text-xs text-rose-300">{{ $message }}</p>@enderror
                    </div>

                    <div>
                        <label class="mb-2 block text-sm text-slate-700 dark:text-slate-300" for="expires_at">Expiry</label>
                        <input id="expires_at" type="date" name="expires_at" value="{{ old('expires_at', $university->expires_at?->format('Y-m-d')) }}" class="w-full rounded-xl border border-slate-300 bg-white text-slate-900 dark:border-white/10 dark:bg-slate-950/60 dark:text-slate-100" />
