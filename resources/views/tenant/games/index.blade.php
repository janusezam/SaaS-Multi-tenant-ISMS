            ...
            <table class="min-w-full divide-y divide-white/10 text-sm">
                ...
                <tbody class="divide-y divide-white/10 text-slate-200">
                    @forelse ($games as $game)
                        <tr>
                            <td class="px-4 py-3">{{ $game->scheduled_at->format('M d, Y h:i A') }}</td>
                            <td class="px-4 py-3">{{ $game->sport?->name }}</td>
                            <td class="px-4 py-3">{{ $game->homeTeam?->name }} vs {{ $game->awayTeam?->name }}</td>
                            <td class="px-4 py-3">{{ $game->venue?->name }}</td>
                            <td class="px-4 py-3 uppercase">{{ $game->status }}</td>
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap gap-2">
                                    <a href="{{ route('tenant.games.edit', $game) }}" class="rounded-md border border-white/10 bg-white/5 px-3 py-1 text-xs hover:bg-white/10">Edit</a>
                                    ...
                                    <form method="POST" action="{{ route('tenant.games.destroy', $game) }}">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    @if (in_array(auth()->user()->role, ['university_admin', 'sports_facilitator'], true))
                                        <form method="POST" action="{{ route('tenant.games.result', $game) }}" class="flex items-center gap-2 rounded-md border border-cyan-300/20 bg-cyan-500/10 px-2 py-1">
                                            @csrf
                                            @method('PATCH')
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-6 text-center text-slate-400">No games scheduled.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
