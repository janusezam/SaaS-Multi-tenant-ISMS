<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold text-slate-100">Edit Sport</h2>
    </x-slot>
        ...
        <div class="rounded-2xl border border-white/10 bg-slate-900/85 p-6">
            <form method="POST" action="{{ route('tenant.sports.update', $sport) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input id="name" name="name" value="{{ old('name', $sport->name) }}" class="w-full rounded-xl border border-white/10 bg-slate-950/60 text-slate-100" required />
                ...
                <div>
                    ...
                    <input id="code" name="code" value="{{ old('code', $sport->code) }}" class="w-full rounded-xl border border-white/10 bg-slate-950/60 text-slate-100" required />
                ...
                <div class="flex gap-3">
                    ...
                    <a href="{{ route('tenant.sports.index') }}" class="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm text-slate-200 hover:bg-white/10">Back</a>
                </div>
            </form>
        </div>
    </div>
