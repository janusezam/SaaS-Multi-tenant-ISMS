                ...
                <tbody class="divide-y divide-white/10 text-slate-200">
                    @forelse ($venues as $venue)
                        <tr>
                            ...
                            <td class="px-4 py-3">{{ $venue->is_active ? 'Active' : 'Inactive' }}</td>
                            <td class="px-4 py-3">
                                <div class="flex gap-2">
                                    <a href="{{ route('tenant.venues.edit', $venue) }}" class="rounded-md border border-white/10 bg-white/5 px-3 py-1 text-xs hover:bg-white/10">Edit</a>
                                    <form method="POST" action="{{ route('tenant.venues.destroy', $venue) }}">
                                        @csrf
                                        @method('DELETE')
