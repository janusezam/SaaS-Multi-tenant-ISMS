                ...
                <tbody class="divide-y divide-white/10 text-slate-200">
                    @forelse ($players as $player)
                        <tr>
                            ...
                            <td class="px-4 py-3">{{ $player->team?->name }}</td>
                            <td class="px-4 py-3">{{ $player->position ?? 'N/A' }}</td>
                            <td class="px-4 py-3">{{ $player->is_active ? 'Active' : 'Inactive' }}</td>
                            <td class="px-4 py-3">
                                <div class="flex gap-2">
                                    <a href="{{ route('tenant.players.edit', $player) }}" class="rounded-md border border-white/10 bg-white/5 px-3 py-1 text-xs hover:bg-white/10">Edit</a>
                                    <form method="POST" action="{{ route('tenant.players.destroy', $player) }}">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-6 text-center text-slate-400">No players available.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
